...
public class MyScanIP extends JFrame implements ActionListener{
	...
	JTextArea ta;
	JScrollPane scroll;
	JButton check,scan,stop,clear,exit;
	...
	public void GUI() {
		...
		ta=new JTextArea();
		ta.setMargin(new Insets(0, 30, 0, 0));
		scroll = new JScrollPane(ta);
		
		check=new JButton("Check");
		scan=new JButton("Scan");if(myip.getHostAddress().equals("127.0.0.1")) scan.setEnabled(false);
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==check) {
			...
			tf3.setText(stringip(decimalip(ba)));
			tf4.setText(stringip(decimalip(na)+1)+" - "+stringip(decimalip(ba)-1));
			if(!myip.getHostAddress().equals("127.0.0.1")) scan.setEnabled(true);
			ta.append("Checked!\n-----------------------------------------------------------\n");
		}
		if(e.getSource()==scan) {
			scan.setEnabled(false);
			...
			for (int ithread = 1; ithread <= numthread; ithread++) {
				int index=ithread;
	...
	void scanning(int index) {
		try {
			for(long i=start+index;i<end;i+=numthread) {
	...
	void updateProgress(String ipaddress) {
		  SwingUtilities.invokeLater(new Runnable() {
		    public void run() {
		    	ta.append(ipaddress+"\t\t"+Double.toString((System.nanoTime()-time)/Math.pow(10,9))+"\n");
		    }
		  });
	}
	...
	void finishScan() {
		  SwingUtilities.invokeLater(new Runnable() {
		    public void run() {
		    	...
		    	if(count==numthread) {
			    	count=0;
			    	if(stopscan==true) ta.append("Stopped!");
			    	else ta.append("Finished!");
			    	ta.append("\t\t"+Double.toString((System.nanoTime()-time)/Math.pow(10,9))+"\n");
			    	ta.append("-----------------------------------------------------------\n");
			    	stopscan=false;
